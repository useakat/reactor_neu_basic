...
set output "plots/EventDistmin_fit2nh_combine_LLL_ERES_ERESNL.eps"
...
set label '{/=30 L=LLL km}' at graph 0.45, graph 0.9
plot \
'DATADIR/events_nh_LLL_0_0.dat' u ($1**2+0.8):($2/(2*$1)) t 'NH' w l lt 2 lc rgb 'blue' lw 3 ,\
'DATADIR/events_ih_LLL_0_0.dat' u ($1**2+0.8):($2/(2*$1)) t 'IH' w l lt 2 lc rgb 'red' lw 3 ,\
'DATADIR/events_nhmin_LLL_0_0.dat' u ($1**2+0.8):($2/(2*$1)) t 'Best Fit to NH data' w l lt 1 lc rgb '#006400' lw 2
...
set format x
#set label '{/Symbol=\144}E_{vis}/E_{vis} = ERES%/{/Symbol=\326}E_{vis} +ERESNL%' at graph 0.05, graph 0.5
set label '(a, b) = (ERES, ERESNL)%' at graph 0.05, graph 0.5
plot \
'DATADIR/events_nh_LLL_ERES_ERESNL.dat' u ($1**2+0.8):($2/(2*$1)) notitle w l lt 2 lc rgb 'blue' lw 3 ,\
'DATADIR/events_ih_LLL_ERES_ERESNL.dat' u ($1**2+0.8):($2/(2*$1)) notitle w l lt 2 lc rgb 'red' lw 3 ,\
'DATADIR/events_nhmin_LLL_ERES_ERESNL.dat' u ($1**2+0.8):($2/(2*$1)) notitle w l lt 1 lc rgb '#006400' lw 2
