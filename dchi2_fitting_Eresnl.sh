#!/bin/bash
    ./dchi2 $Lmin $Lmax $ndiv $P $V $R $Y ${Eres} $Eres_nl} ${mode} 0 ${ifixL} ${ifluc}
    if [ ${ifixL} -eq 0 ];then
	...
	mv dchi2_error_nh.dat dchi2_error_nh_${Eres}_${Eres_nl}.dat >/dev/null 2>&1
	mv dchi2_error_ih.dat dchi2_error_ih_${Eres}_${Eres_nl}.dat >/dev/null 2>&1
    fi